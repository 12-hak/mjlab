...
class UniformVelocityCommand(CommandTerm):
    ...
    def nudge_command(self, x: float = 0.0, y: float = 0.0, yaw: float = 0.0):
        """Nudge the current manual command."""
        self.manual_override = True
        self.resampling_active = False
        self.vel_command_b[:, 0] = torch.clamp(
            self.vel_command_b[:, 0] + x, *self.cfg.ranges.lin_vel_x
        )
        self.vel_command_b[:, 1] = torch.clamp(
            self.vel_command_b[:, 1] + y, *self.cfg.ranges.lin_vel_y
        )
        self.vel_command_b[:, 2] = torch.clamp(
            self.vel_command_b[:, 2] + yaw, *self.cfg.ranges.ang_vel_z
        )

    def reset_manual_override(self):
        """Return to automatic resampling."""
        self.manual_override = False
        self.resampling_active = True
        self._resample(torch.tensor(range(self.num_envs), device=self.device))

    def compute(self, dt: float) -> None:
        """Update metrics and commands, optionally skipping resampling."""
