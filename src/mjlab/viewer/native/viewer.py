...
class NativeMujocoViewer(BaseViewer):
    ...
    def _safe_key_callback(self, key: int) -> None:
        ...
        if key == KEY_ENTER:
            ...
            self.request_action("MOVE_FORWARD")
        elif key == KEY_DOWN:
            self.request_action("MOVE_BACKWARD")
        elif key == KEY_LEFT:
            self.request_action("TURN_LEFT")
        elif key == KEY_RIGHT:
            self.request_action("TURN_RIGHT")
        elif key == KEY_X:
            self.request_action("RESET_MANUAL")
        elif key == KEY_W:
            self.request_action("MOVE_LEFT")
        elif key == KEY_S:
            self.request_action("MOVE_RIGHT")
    ...
    def _handle_custom_action(self, action, payload) -> bool:
        if action == ViewerAction.PREV_ENV and self.env.unwrapped.num_envs > 1:
            ...
            self._clear_histories()
            self.log(
                f"[INFO] Switched to environment {self.env_idx}", VerbosityLevel.INFO
            )
            return True
        ...
        if self.cfg and hasattr(self.cfg, "origin_type"):
            if self.cfg.origin_type == self.cfg.OriginType.WORLD:
                ...
                if not self.cfg.asset_name:
                    raise ValueError(
                        "Asset name must be specified for ASSET_ROOT origin type"
                    )
                ...
                if not self.cfg.asset_name or not self.cfg.body_name:
                    raise ValueError(
                        "asset_name/body_name required for ASSET_BODY origin type"
                    )
                robot: Entity = self.env.unwrapped.scene[self.cfg.asset_name]
            ...
            self.viewer.cam.azimuth = getattr(
                self.cfg, "azimuth", self.viewer.cam.azimuth
            )
            self.viewer.cam.distance = getattr(
                self.cfg, "distance", self.viewer.cam.distance
            )
        else:
            self.viewer.cam.type = mujoco.mjtCamera.mjCAMERA_FREE.value
            self.viewer.cam.fixedcamid = -1
